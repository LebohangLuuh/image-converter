<script lang="ts">
  ...
  async function handleFileUpload(event: Event) {
    try {
      ...
      if (!isValidType) {
        alert("Please select a valid video file (MP4, AVI, MOV, MKV, WebM, etc.)");
        return;
      }

      // Dynamic size limit based on quality setting
      const maxSizes = { high: 100, medium: 150, low: 200 };
      const maxSize = maxSizes[videoQuality as keyof typeof maxSizes] * 1024 * 1024;
      
      if (file.size > maxSize) {
        alert(`File is too large. Maximum size for ${videoQuality} quality is ${maxSizes[videoQuality as keyof typeof maxSizes]}MB.`);
        return;
      }
  ...
  async function convertVideo(): Promise<void> {
    ...
    try {
      ...
      try {
        ...
        console.log(`Input file written successfully: ${verifyInput.length} bytes`);
      } catch (verifyError) {
      ...
      try {
        ...
        // This is expected to "fail" but gives us file info in the logs
        console.log("File probe completed (expected error)");
      }

      // Build robust conversion command with fallback options
      const quality = qualitySettings[videoQuality as keyof typeof qualitySettings];
      
      // Start with basic, reliable command structure
      ...
      // Add scaling if needed (simplified)
      if (quality.maxWidth && quality.maxWidth < 1920) {
        ...
        finalCommand.splice(outputIndex, 0, "-vf", `scale=${quality.maxWidth}:-2`);
      }
      ...
      conversionTimeout = setTimeout(() => {
        throw new Error(`Conversion timeout after ${timeoutDuration / 60000} minutes`);
      }, timeoutDuration);
      ...
      try {
        ...
        // First conversion attempt failed, try with simpler parameters
        console.warn("First conversion attempt failed:", execError);
        
        loadingMessage = "Retrying with simpler encoding...";
        
        // Fallback command with minimal parameters
        ...
        try {
          ...
          throw new Error(`Both conversion attempts failed. Primary: ${execError}. Fallback: ${fallbackError}`);
        }
      }
      ...
      try {
        ...
        // Try to list files in the virtual filesystem for debugging
        try {
        ...
        throw new Error(`Failed to read converted file: ${msg}. The conversion may have failed silently.`);
      }
      ...
      if (outputData.length < 1000) {
        ...
        if (selectedFormat === "mp4") {
          ...
          hasValidHeader = outputData[0] === 0x1A && outputData[1] === 0x45 && outputData[2] === 0xDF && outputData[3] === 0xA3;
        } else if (selectedFormat === "avi") {
          // Check for AVI RIFF signature
          hasValidHeader = outputData[0] === 0x52 && outputData[1] === 0x49 && outputData[2] === 0x46 && outputData[3] === 0x46;
        }
        
        if (!hasValidHeader) {
          throw new Error(`Conversion produced invalid ${selectedFormat.toUpperCase()} file (${outputData.length} bytes). The file may be corrupted.`);
        }
      }
      ...
      downloadConvertedFile(blob, `converted_${uploadedFile.name.split('.')[0]}.${selectedFormat}`);

      console.log(`Conversion completed successfully. Output size: ${formatFileSize(outputData.length)}`);
      ...
      const isMemoryError = /memory|out of bounds|wasm/i.test(errorMessage) || 
                           conversionLog.some(log => /memory|out of bounds/i.test(log));
      
      const isCodecError = /codec|encoder|decoder/i.test(errorMessage) ||
                          conversionLog.some(log => /codec|encoder|decoder/i.test(log));
      
      const isEmptyOutputError = errorMessage.includes("empty output") || 
      ...
      const recentLogs = conversionLog.slice(-3).filter(log => 
      ...
      progress = 0;
      estimatedTime = "";
      loadingMessage = isFFmpegLoaded ? "Ready for next conversion" : loadingMessage;
    }
  }
  ...
  function getEstimatedConversionTime(fileSizeBytes: number, quality: string): string {
    // Rough estimates based on typical conversion rates
    const baseTimePerMB = { high: 15, medium: 8, low: 4 }; // seconds per MB
    const fileSizeMB = fileSizeBytes / (1024 * 1024);
    const estimatedSeconds = fileSizeMB * baseTimePerMB[quality as keyof typeof baseTimePerMB];
...
<div class="card w-[50%] pr-10 pb-10 pt-10 ml-[30%] mt-[-10%]">
  <div class="mt-0 ml-10">
    ...
    {:else}
      ...
      <div class="mt-5 space-y-4">
        <!-- Quality Selection (moved up for better UX) -->
        <div>
          <label for="quality-select" class="format-label text-primary-500 font-semibold block mb-2">
            Quality & Speed:
          </label>
          <select 
            id="quality-select" 
        ...
        <!-- Format Selection -->
        <div>
          <label for="format-select" class="format-label text-primary-500 font-semibold block mb-2">
            Output Format:
          </label>
          <select 
            id="format-select" 
      ...
      {#if uploadedFile}
        <div class="mt-4 p-3 bg-surface-100-800-token rounded-lg">
          ...
          <div class="text-xs text-surface-600-300-token mt-1 space-y-1">
            <div>Size: {formatFileSize(uploadedFile.size)} | Type: {uploadedFile.type}</div>
            {#if selectedFormat && videoQuality}
              <div>Estimated conversion time: ~{getEstimatedConversionTime(uploadedFile.size, videoQuality)}</div>
            {/if}
          </div>
        </div>
      {/if}
      ...
      {#if isConverting}
        <div class="mt-4 p-4 bg-primary-100-800-token rounded-lg">
          ...
          {#if conversionLog.length > 0}
            <details class="mt-2">
              <summary class="text-xs cursor-pointer text-surface-600-300-token hover:text-surface-500-400-token">
                Technical Details ({conversionLog.length} logs)
              </summary>
              <div class="mt-2 text-xs font-mono bg-surface-200-700-token p-2 rounded max-h-24 overflow-y-auto border">
  ...
  >
    {#if !isFFmpegLoaded}
      Loading FFmpeg... {loadingProgress > 0 ? `${Math.round(loadingProgress)}%` : ''}
